...
Begin VB.Form ofcsearch 
   ...
   Begin VB.ComboBox cmbOTakenBy 
      ...
      EndProperty
      Height          =   450
      ItemData        =   "ofcsearch.frx":0000
      Left            =   10440
      List            =   "ofcsearch.frx":0002
      TabIndex        =   13
      Top             =   6240
      Width           =   3495
   End
   ...
   Begin VB.Image i1 
      Height          =   10935
      Left            =   0
      Picture         =   "ofcsearch.frx":0004
      Stretch         =   -1  'True
      Top             =   0
      Width           =   20295
   End
End
Attribute VB_Name = "ofcsearch"
...
Call dbconnection2
    recset.Close
    recset.Open "select * from OrderFromCustomer where Order_Id = '" & txtOId.Text & "'", dbconn, adOpenDynamic, adLockOptimistic
...
Dim ans As String
    If recset.EOF = True Then
       ...
       Exit Sub
    Else
        ans = MsgBox("Do you want to Update this Record, then click Yes", vbYesNo)
        If ans = vbYes Then
            Call updateRecord2
            recset.Update
            MsgBox "Your record is Updated..."
            Call closeDB
            Call ClearAll
            Exit Sub
        End If
        If ans = vbNo Then
           recset.CancelUpdate
            MsgBox "Your Updation is Cancelled!!!"
            Call closeDB
            Exit Sub
        End If
    Exit Sub
...
Private Sub showRecord2()
    txtOId.Text = recset.Fields("Order_Id").Value
    ...
    If recset.Fields("Payment_Mode").Value = "Card" Then
            p4.Value = True
    End If
    cmbOTakenBy.Text = recset.Fields("Order_Taken_By").Value
